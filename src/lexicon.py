...
DEFAULT_HOST = os.environ.get("LEXICON_HOST", "127.0.0.1")
DEFAULT_PORT = int(os.environ.get("LEXICON_PORT", "48624"))
DEFAULT_TIMEOUT = 10.0

# Path mapping: Lexicon stores paths as seen by the local machine.
# Override via LEXICON_PATH_PREFIX / NAS_MUSIC_PREFIX env vars.
LEXICON_PATH_PREFIX = os.environ.get(
    "LEXICON_PATH_PREFIX", "/Volumes/Macintosh HD/Users/Shared/SynologyDrive/Database/"
)
NAS_PATH_PREFIX = os.environ.get("NAS_MUSIC_PREFIX", "/volume1/music/Database/")

# NAS connection details — loaded from env vars with safe defaults
NAS_HOST = os.environ.get("NAS_HOST", "localhost")
NAS_SSH_PORT = int(os.environ.get("NAS_SSH_PORT", "7844"))
NAS_USER = os.environ.get("NAS_USER", "admin")
NAS_SSH_KEY = Path(os.environ.get("NAS_SSH_KEY", str(Path.home() / ".ssh" / "id_ed25519")))
...
def lexicon_to_nas_path(lexicon_path: str) -> str:
    ...
    Lexicon: /Volumes/Macintosh HD/Users/<user>/SynologyDrive/Database/Artist/...
...
def copy_from_nas(nas_path: str, local_path: Path) -> Path:
    """
    Copy a file from NAS to local path.

    If running inside the Docker container on the NAS (``/music`` volume mounted),
    copies directly from the filesystem. Otherwise, falls back to SSH cat.
    """
    local_path.parent.mkdir(parents=True, exist_ok=True)

    # Direct mode: try local mount first (Docker container on NAS)
    if nas_path.startswith("/volume1/music/") and Path("/music").is_dir():
        direct_src = Path(nas_path.replace("/volume1/music", "/music", 1))
        if direct_src.exists():
            import shutil
            shutil.copy2(str(direct_src), str(local_path))
            logger.info(
                f"Direct copy from NAS mount: {direct_src} -> {local_path} "

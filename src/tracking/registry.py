...
class RenderRegistry:
    ...
    def _init_db(self):
        with self._conn() as conn:
            ...
            """)
            # Audio fingerprint table for track identification
            conn.execute("""
            ...
            conn.execute("""
                ...
                ON audio_fingerprints(fingerprint)
            """)
    ...
    def store_fingerprint(
        ...
        """Store an audio fingerprint for track identification."""
        now = datetime.now().isoformat()
        with self._conn() as conn:
            conn.execute("""
                INSERT INTO audio_fingerprints
                    (fingerprint, audio_path, track_name, artist, duration_sec,
                     file_hash, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(fingerprint) DO UPDATE SET
                    audio_path = ?, track_name = ?, artist = ?,
                    duration_sec = ?, file_hash = ?
            """, (fingerprint, audio_path, track_name, artist, duration_sec,
                  file_hash, now,
                  audio_path, track_name, artist, duration_sec, file_hash))

    def lookup_fingerprint(self, fingerprint: str) -> Optional[dict]:
        """Look up a track by its audio fingerprint."""
        with self._conn() as conn:
            row = conn.execute(
                "SELECT * FROM audio_fingerprints WHERE fingerprint = ?",
                (fingerprint,),
            ).fetchone()
    ...
    def get_all_fingerprints(self) -> list[dict]:
        ...
        with self._conn() as conn:
            rows = conn.execute(
                "SELECT * FROM audio_fingerprints ORDER BY track_name"
            ).fetchall()
            return [dict(r) for r in rows]

...
def stitch_videos(
    ...
    # Too many segments for a single xfade chain (ffmpeg struggles with 15+)
    # Batch stitch in groups of 10, then stitch the batches
    MAX_BATCH = 10
    if len(segments) > MAX_BATCH:
        logger.info(
            f"Batching stitch: {len(segments)} segments in groups of {MAX_BATCH}"
        )
        batch_outputs = []
        for batch_start in range(0, len(segments), MAX_BATCH):
            batch_segs = segments[batch_start : batch_start + MAX_BATCH]
            batch_durs = []
            for seg in batch_segs:
                info = get_video_info(seg)
                batch_durs.append(info["duration"])
            batch_target = sum(batch_durs) - crossfade_seconds * max(0, len(batch_segs) - 1)

            with tempfile.NamedTemporaryFile(suffix=".mp4", delete=False) as tmp:
                batch_out = Path(tmp.name)
            # Recursively stitch each batch (always <= MAX_BATCH segments)
            stitch_videos(
                batch_segs, batch_out, batch_target,
                crossfade_seconds=crossfade_seconds, fps=fps,
            )
            batch_outputs.append(batch_out)

        # Now stitch the batches (may recurse again if > MAX_BATCH batches)
        return stitch_videos(
            batch_outputs, output_path, target_duration,
            crossfade_seconds=crossfade_seconds, fps=fps,
        )
    ...
    #
    # All inputs are first normalised to a common timebase and fps so that
    # xfade does not fail on timebase mismatches (e.g. Google Veo videos at
    # 1/12288 vs libx264 at 1/15360).
    ...
    if len(segments) == 2:
        ...
        _run_ffmpeg([
            ...
            (
                ...
                f"[v0][v1]xfade=transition=fade:duration={crossfade_seconds}:offset={offset}[outv]"
            ),
            "-map", "[outv]",
            "-t", str(target_duration),
            "-c:v", "libx264",
        ...
        # 1. Normalise each input stream to common timebase + fps
        # 2. Chain xfade filters
        filter_parts = []

        # Normalise all inputs
        for idx in range(len(segments)):
            filter_parts.append(f"[{idx}:v]settb=AVTB,fps={fps}[v{idx}]")
        ...
        if running_offset < 0:
            running_offset = 0
        filter_parts.append(
            f"[v0][v1]xfade=transition=fade:duration={crossfade_seconds}"
        ...
        filter_parts.append(f"[{final_label}]null[outv]")

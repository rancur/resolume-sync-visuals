...
class TestSupportedModels:
    ...
    def test_known_models_present(self):
        ...
        assert "kling-v1-5-pro" in SUPPORTED_VIDEO_MODELS
        assert "minimax" in SUPPORTED_VIDEO_MODELS
        assert "runway-gen3" in SUPPORTED_VIDEO_MODELS
        assert "wan2.1-480p" in SUPPORTED_VIDEO_MODELS
...
class TestPlanSegments:
    ...
    def test_very_short_chunks_absorbed(self, sample_analysis):
        ...
        # Create analysis with a phrase whose beat-quantized splits leave
        # a tiny remainder (< 1s). At 128 BPM, beat = 0.46875s, 8 beats = 3.75s.
        # A phrase of 7.8s: splits at 3.75, then 7.5; remainder = 0.3s < 1.0 -> absorbed.
        analysis = {
            "phrases": [
                {"start": 0.0, "end": 7.8, "beats": 16, "energy": 0.5,
        ...
        segments = _plan_segments(analysis, max_segment_duration=10)
        # The tiny remainder should be absorbed into the last segment
...
class TestEstimateCost:
    ...
    def test_basic_cost_estimate(self):
        ...
        assert cost["total_cost"] == round(cost["video_cost"] + cost["keyframe_cost"], 2)
        # 180s / 10s max = 18 segments, each 10s * $0.10/s = $1.00
        assert cost["num_segments"] == 18
        assert cost["video_cost"] == 18.0
    ...
    def test_high_quality_keyframes_cost_more(self):
        ...
        # High = $0.08, draft = $0.04 (1 keyframe per song)
...
class TestAnimateKeyframe:
    ...
    def test_replicate_model_routes_to_replicate(self, tmp_dir):
        ...
        config = VideoGenerationConfig(
            video_model="fake-replicate-model", replicate_token="test-token"
        )
        keyframe = tmp_dir / "keyframe.png"
        keyframe.write_bytes(b"\x89PNG" + b"\x00" * 500)
        ...
        with patch.dict("src.generator.video_pipeline.SUPPORTED_VIDEO_MODELS", {"fake-replicate-model": fake_spec}):
            with patch(
...
class TestGenerateFullSongVideo:
    ...
    def test_full_pipeline_runs(self, sample_analysis, tmp_dir):
        ...
        # 6 phrases of 30s each, split at model's max_duration (10s)
        # 30s / 10s = 3 segments per phrase, 6 phrases = 18 segments
    ...
    def test_chaining_falls_back_to_fresh_keyframe(self, sample_analysis, tmp_dir):
        ...
        # All keyframes should be freshly generated (no chaining worked)
        # 6 phrases x 30s each, split at 10s max = 18 segments
...
class TestCostSafety:
    """Test cost cap, expensive model blocking, and model recommendation."""

    def test_max_cost_per_song_exists(self):
        assert MAX_COST_PER_SONG == 30.0

    def test_veo2_is_expensive(self):
        assert "veo2" in EXPENSIVE_MODELS
        assert "veo3" in EXPENSIVE_MODELS

    def test_validate_model_cost_blocks_expensive_full_song(self):
        """Veo2 for a 4-minute song should be blocked."""
        with pytest.raises(ValueError, match="would cost"):
            validate_model_cost("veo2", 263.0)

    def test_validate_model_cost_allows_short_veo2(self):
        """Veo2 for a very short clip should be allowed."""
        # 8s clip = 1 segment * $0.50/s * 8s = $4.00, under $50
        validate_model_cost("veo2", 8.0)  # Should not raise

    def test_validate_model_cost_allows_non_expensive(self):
        """Non-expensive models should never be blocked."""
        validate_model_cost("kling-v1", 300.0)  # Should not raise
        validate_model_cost("minimax", 600.0)  # Should not raise

    def test_recommend_model_within_budget(self):
        model = recommend_model(263.0, max_budget=30.0)
        spec = SUPPORTED_VIDEO_MODELS[model]
        import math
        segments = math.ceil(263.0 / spec["max_duration"])
        cost = segments * spec["cost_per_sec"] * spec["max_duration"]
        assert cost <= 30.0

    def test_recommend_model_picks_cheapest(self):
        # For a very tight budget, should pick a cheap model
        model = recommend_model(263.0, max_budget=5.0)
        assert model in SUPPORTED_VIDEO_MODELS

    def test_recommend_model_returns_something_always(self):
        # Even with $0 budget, should return something
        model = recommend_model(263.0, max_budget=0.0)
        assert model in SUPPORTED_VIDEO_MODELS

    def test_estimate_cost_263s_kling_v1(self):
        """263s song with Kling v1 (10s segments) should be reasonable."""
        config = VideoGenerationConfig(video_model="kling-v1")
        cost = estimate_cost(263.0, config)
        # 263s / 10s = 27 segments * 10s * $0.10/s = $27.00 + $0.08 keyframe
        assert cost["num_segments"] == 27
        assert cost["video_cost"] == 27.0
        assert cost["total_cost"] == 27.08

    def test_estimate_cost_263s_minimax(self):
        """263s song with MiniMax (6s segments) should be cheap."""
        config = VideoGenerationConfig(video_model="minimax")
        cost = estimate_cost(263.0, config)
        # 263s / 6s = 44 segments * 6s * $0.05/s = $13.20 + $0.08 keyframe
        assert cost["num_segments"] == 44
        assert cost["video_cost"] == 13.2
        assert cost["total_cost"] == 13.28

    def test_no_sub_chunking_in_segments(self, sample_analysis):
        """Segments should never exceed max_segment_duration.
        This ensures 1 segment = 1 API call with no sub-chunking."""
        for model_name, spec in SUPPORTED_VIDEO_MODELS.items():
            max_dur = spec["max_duration"]
            segments = _plan_segments(sample_analysis, max_dur)
            for seg in segments:
                assert seg["duration"] <= max_dur + 0.5, (
                    f"Model {model_name}: segment duration {seg['duration']:.1f}s "
                    f"exceeds max {max_dur}s"
                )

    def test_cost_cap_rejects_over_budget(self, sample_analysis, tmp_dir):
        """When estimated cost exceeds max_cost, generation should be rejected."""
        config = VideoGenerationConfig(
            video_model="kling-v1",
            openai_key="test-key",
            fal_key="test-fal-key",
            work_dir=str(tmp_dir / "work"),
            max_cost=5.0,  # 180s kling-v1 costs $18, exceeds $5 cap
        )
    ...
    def test_cost_cap_allows_within_budget(self, sample_analysis, tmp_dir):
        """When estimated cost is within max_cost, generation should proceed."""
        config = VideoGenerationConfig(
            video_model="cogvideox",  # cheapest: $0.02/s * 6s = $0.12 per segment
            openai_key="test-key",
            fal_key="test-fal-key",
            work_dir=str(tmp_dir / "work"),
            max_cost=30.0,
        ...
        # Use the work/clips dir that generate_full_song_video creates,
        # so the clips persist and aren't cleaned as temp files.
        def make_clip(keyframe, prompt, duration, config_arg):
            i = clip_index[0]
            clip_index[0] += 1
            # Write to the clips dir that the pipeline creates
            clips_dir = Path(config.work_dir) / "clips"
            clips_dir.mkdir(parents=True, exist_ok=True)
            clip = clips_dir / f"api_clip_{i:04d}.mp4"
            clip.write_bytes(b"\x00" * 5000)
            return clip
        ...
        with patch(
            ...
            "src.generator.video_pipeline._animate_keyframe", side_effect=make_clip
        ), patch(
            "src.generator.video_pipeline._extract_last_frame", return_value=fake_frame
        ), patch(
            "src.generator.video_pipeline._stitch_segments", return_value=output
        ...
        # cogvideox: 180s / 6s = 30 segments, all should generate within budget
        assert clip_index[0] == 30

    def test_expensive_model_blocked_for_full_song(self, sample_analysis, tmp_dir):
        """Veo2 should be blocked for full song generation."""
        config = VideoGenerationConfig(
            video_model="veo2",
            openai_key="test-key",
            fal_key="test-fal-key",
